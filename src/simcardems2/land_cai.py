...
def _cat50(scale_HF_cat50_ref, Beta1, lmbda, cat50_ref, dt, scheme: Scheme):
    if scheme == Scheme.analytic:
        return scale_HF_cat50_ref * (
            Beta1 * ((np.where((lmbda < 1.2), lmbda, 1.2)) - 1) + cat50_ref
        )
...
_parameters = {
    ...
    "ntm": 2.4,  # Parameters from CaTrpn split. # TODO: check. ntm = 2.2 or 5 in Land et al. 2017 vs 2.4 in .ode files  # noqa: E501
    "ku": 0.04,  # Parameters from CaTrpn split. # TODO: check. ku = 1 in Land et al. 2017 # noqa: E501
...
class LandModel(pulse.ActiveModel):
    ...
    def calculate_cat50(self):
        ...
        self._cat50.vector()[:] = _cat50(
            ...
            lmbda=self.lmbda_prev.vector().get_local(),  # Explicit update, to be consistent with other split schemes # noqa: E501
