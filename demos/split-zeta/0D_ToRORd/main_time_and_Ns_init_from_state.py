...
save_traces = True
run_full_model = True
TOR = True # Use ToRORd_dynCl_endo
...
#Ns = np.array([1, 2, 4, 6, 8, 10, 20, 50, 100, 200, 400, 800])
Ns = np.array([1])
...
if TOR == False:
    mech_state_indices = [14, 31] #Zetas, zetaw
elif TOR:
    mech_state_indices = [10, 24,43]#Zetas, zetaw, cd
...
for N in Ns:
    ...
    timings_ep_steps = []
    timings_mech_steps = []
    timings_calculate_lmbda = []
    timings_mech_to_ep_transfer = []
    for i, ti in enumerate(t):
        ...
        if i % N != 0:
            count += 1
            timing_calc_lmbda_start = time.perf_counter()
            p, p_mechanics, p_ep, prev_lmbda = update_lambda_and_dlambda(ti+dt, prev_lmbda, dt)
            timings_calculate_lmbda.append(time.perf_counter()-timing_calc_lmbda_start)
        ...
        timing_extract_mv_missing_start =  time.perf_counter()
        # Update missing values for the EP model
        ep_missing_values[:] = mv_mechanics(
            t, y_mechanics, p_mechanics, mechanics_missing_values
        )
        timings_mech_to_ep_transfer.append(time.perf_counter() - timing_extract_mv_missing_start)
    ...
    with open(f"timings_N{N}", "w") as f:        
        ...
        f.write("Calc lmbda times\n")
        np.savetxt(f, timings_calculate_lmbda)
        f.write("timings_mech_to_ep_transfer\n")
        np.savetxt(f, timings_mech_to_ep_transfer)
        f.write("Total time\n")
