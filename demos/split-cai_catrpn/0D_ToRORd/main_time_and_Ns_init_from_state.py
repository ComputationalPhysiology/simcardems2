...
mech_state_indices = [10, 11, 12, 13, 24, 43]
...
Ns = np.array([1, 2, 4, 5, 6, 8, 10, 20,30, 40, 42, 43,100, 200, 400, 800])
...
# Load the model
ode = gotranx.load_ode("ToRORd_dynCl_endo_catrpnsplit.ode")

mechanics_comp = ode.get_component("mechanics")
mechanics_ode = mechanics_comp.to_ode()

ep_ode = ode - mechanics_comp
ep_file = Path("ToRORd_dynCl_endo_catrpnsplit_ep.py")
...
if not ep_file.is_file() or rebuild:
    ...
    Path("ToRORd_dynCl_endo_catrpnsplit_mechanics.py").write_text(code_mechanics)
    Path("ToRORd_dynCl_endo_catrpnsplit.py").write_text(code)



# Import ep, mechanics and full model
import ToRORd_dynCl_endo_catrpnsplit_ep
import ToRORd_dynCl_endo_catrpnsplit_mechanics
import ToRORd_dynCl_endo_catrpnsplit

model = ToRORd_dynCl_endo_catrpnsplit.__dict__
ep_model = ToRORd_dynCl_endo_catrpnsplit_ep.__dict__
mechanics_model = ToRORd_dynCl_endo_catrpnsplit_mechanics.__dict__
